...
def plot_historical_prices(ax):
    ...
    if historical_data:
        # Berechne Heikin-Ashi Close-Preise
        ha_closes = []
        for i in range(len(historical_data)):
            ha_close = (historical_data[i][1] + historical_data[i][2] + 
                       historical_data[i][3] + historical_data[i][4]) / 4
        ...
        # Dünne Heikin-Ashi Linie (0.75 statt 1.5)
        ax.plot(dates, ha_closes, color=theme_color, linewidth=1.0, alpha=0.9)  # alpha für Transparenz
        
        # Formatierung
        ax.spines[:].set_color(theme_color)
        ax.set_ylabel('EUR', color=theme_color, fontsize=8)  # Kleinere Schrift
        ax.tick_params(axis='both', colors=theme_color, labelsize=9)  # Kleinere Achsenbeschriftung
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%H:%M' if current_time_range == '12h' else '%d-%m'))
        ax.grid(color=theme_color, linestyle=':', linewidth=1.0, alpha=0.5)  # Dünnere Grid-Linien
