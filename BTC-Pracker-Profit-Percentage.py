...
AVG_PRICE_FILE = "avg_price.txt"  # Neue Datei für AVG Price
...
# NEUE FUNKTIONEN FÜR AVG PRICE UND GEWINNBERECHNUNG
...
def update_profit_display():
    """Aktualisiere die Gewinn/Verlust-Anzeige."""
    current_price = get_bitcoin_price_eur()
    avg_price = load_avg_price()
    
    if current_price is not None and avg_price > 0:
        profit_percentage = calculate_profit_percentage(avg_price, current_price)
        profit_color = "#6FAB65" if profit_percentage >= 0 else "#BD5959"
        profit_label.config(text=f"{profit_percentage:+.2f}%", fg=profit_color)
    else:
        profit_label.config(text="N/A", fg="grey")
    
    # Alle 10 Sekunden aktualisieren
    profit_label.after(10000, update_profit_display)
...
def open_options():
    ...
    # NEUE AVG PRICE EINGABE IN OPTIONEN
    tk.Label(options_window, text="AVG Price (€):", bg="#212121", fg="white").pack(pady=5)
    avg_price_var = tk.StringVar(value=str(load_avg_price()))
    avg_price_entry = tk.Entry(options_window, textvariable=avg_price_var, bg="#212121", fg="white", insertbackground="white")
    ...
    def save_avg_price_from_options():
        try:
            ...
            # Sofortige Aktualisierung der Gewinnanzeige
            update_profit_display()
        except ValueError:
            pass  # Ungültige Eingabe ignorieren

    # Save AVG Price Button
    save_avg_button = tk.Button(options_window, text="Save AVG Price", command=save_avg_price_from_options, bg=theme_color, fg="black")
    ...
    def save_options():
        ...
        save_avg_price_from_options()  # AVG Price speichern
    ...
    # Bind hover effects to Save AVG Price button
    save_avg_button.bind("<Enter>", lambda event: on_enter_button(event, save_avg_button))
    save_avg_button.bind("<Leave>", lambda event: on_leave_button(event, save_avg_button))
...
def update_conversion(event=None):
    try:
        ...
        # EUR Wert immer in grau anzeigen
        eur_value_label.config(text=f"{eur_value:.2f} EUR")
        
        # Berechne Gewinn/Verlust für Prozent-Anzeige
        avg_price = load_avg_price()
        if avg_price > 0 and btc_to_eur_rate is not None:
            profit_percentage = calculate_profit_percentage(avg_price, btc_to_eur_rate)
            profit_color = "#6FAB65" if profit_percentage >= 0 else "#BD5959"
            # Nur die Prozente mit Farbe anzeigen
        ...
        eur_value_label.config(text="Invalid input")
...
# Start the price update loop with animation
    ...
    # EUR Wert Label (bleibt grau)
    eur_value_label = tk.Label(conversion_frame, text="0 EUR", bg="#212121", fg="grey", font=("Arial", 10))
    eur_value_label.pack(side="left")

    # Prozent Label (wechselt zwischen grün/rot)
...
def update_profit_periodically():
    """Aktualisiert regelmäßig nur die Prozent-Anzeige"""
    try:
        btc_amount = float(btc_entry.get())
        btc_to_eur_rate = get_bitcoin_price_eur()
        
        if btc_to_eur_rate is not None:
            # EUR Wert bleibt unverändert
            eur_value = btc_amount * btc_to_eur_rate
            eur_value_label.config(text=f"{eur_value:.2f} EUR")
            
            # Nur Prozente aktualisieren
            avg_price = load_avg_price()
            if avg_price > 0:
                profit_percentage = calculate_profit_percentage(avg_price, btc_to_eur_rate)
                profit_color = "#6FAB65" if profit_percentage >= 0 else "#BD5959"
    ...
    root.after(10000, update_profit_periodically)

# Starte die Aktualisierung
update_profit_periodically()
